...
if __name__ == '__main__':
    ...
    for i, mut_type in enumerate(mutation_types):
        ...
        pre_omicron = np.log(syn_mut_counts_XY_df['actual_count_pre_omicron'].values + 0.5)
        omicron = np.log(syn_mut_counts_XY_df['actual_count_omicron'].values + 0.5)

        # Calculate ratio of medians
        pre_omicron_median = np.median(syn_mut_counts_XY_df['actual_count_pre_omicron'].values)
        omicron_median = np.median(syn_mut_counts_XY_df['actual_count_omicron'].values)
        median_ratio = omicron_median / pre_omicron_median

        # Calculate Pearson correlation
        corr, _ = pearsonr(pre_omicron, omicron)

        # Plot on the respective subplot
        axes[i].scatter(pre_omicron, omicron, alpha=0.05, edgecolors='none', s=20, color='blue')
        axes[i].set_title(f'{mut_type} (r = {corr:.2f}, ratio of medians = {median_ratio:.2f})')
        if (i+1) % 3 == 0:
            axes[i].set_xlabel('Log-count pre-Omicron')
        if i < 3:
            axes[i].set_ylabel('Log-count Omicron')
    ...
    # Save plot
    plt.tight_layout()
    plt.savefig('../results/pre_vs_post-omicron.pdf')
    plt.show()
